...
public class P2J8
{
    public static void hittingIntegerPowers(int a, int b, int t, int[] out){
     ...
     while (expect) {
         ...
         if(diff.compareTo(num1) <= 0 || diff.compareTo(num2) <= 0) {
             out[0] = pa; out[1] = pb; //raised to power of a and b
             break;
         }
    ...
    public static BigInteger nearestPolygonalNumber(BigInteger n, int s){
        ...
        BigInteger first_Diff = n.subtract(sequence(a, s)).abs();
        BigInteger second_Diff = n.subtract(sequence(b, s)).abs();
        
        if (first_Diff.compareTo(second_Diff) <= 0){
            return sequence(a, s);
        }
        else{
            return sequence(b, s);
